/**
 * HTTP API for encrypt/crack. Falls back to local simulation + scripted solver demo.
 * Set window.ENIGMA_API_BASE = "http://localhost:5000" before loading app to point at a real backend.
 */
...
function getBase() {
  if (typeof window !== "undefined" && window.ENIGMA_API_BASE) {
    return String(window.ENIGMA_API_BASE).replace(/\/$/, "");
  }
  return "";
}
...
export async function postCrack(payload) {
  ...
  if (!res.ok) throw new Error(`crack failed: ${res.status}`);
  return res.json();
}
...
/**
 * Deterministic mock SAT result aligned with current UI (demo when no server).
 */
